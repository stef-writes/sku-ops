...
import { BarChart3, TrendingUp, Package, DollarSign, Calendar as CalendarIcon, Download, Layers, Briefcase, ArrowRight } from "lucide-react";
import { format } from "date-fns";
import { AreaChart } from "@tremor/react";
import { valueFormatter } from "@/lib/chartConfig";
import { DATE_PRESETS } from "@/lib/constants";
import { PageSkeleton } from "@/components/LoadingSkeleton";
import { useReportSales, useReportInventory, useReportTrends, useReportMargins, useReportJobPl } from "@/hooks/useReports";
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${isEmpty ? "bg-red-400" : "bg-orange-400"}`} style={{ width: `${pct}%` }} />
              </div>
        ...
        <div key={i} className="py-3 flex items-center gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1.5 h-1.5 bg-slate-100 rounded-full overflow-hidden w-full">
              <div className={`h-full rounded-full transition-all duration-700 ${isHigh ? "bg-emerald-400" : isLow ? "bg-orange-400" : "bg-blue-400"}`} style={{ width: `${Math.min(p.margin_pct, 100)}%` }} />
            </div>
          </div>
...
const Reports = () => {
  ...
  const { data: jobPlReport, isLoading: jobPlLoading } = useReportJobPl(dateParams);
  ...
  const handleExportCSV = () => {
    ...
    if (activeTab === "sales" && salesReport) {
      ...
      if (salesReport.top_products?.length) { rows.push([]); rows.push(["Product", "Revenue", "Quantity"]); salesReport.top_products.forEach((p) => rows.push([p.name, p.revenue, p.quantity])); }
  ...
  const loading = salesLoading && invLoading && trendsLoading && jobPlLoading;
    ...
    <div className="p-8" data-testid="reports-page">
      <div className="flex items-start justify-between mb-8">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(dateRange.from || dateRange.to) && <button onClick={() => setDateRange({ from: null, to: null })} className="text-xs text-slate-400 hover:text-slate-600" data-testid="clear-date-btn">Clear</button>}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="job-pl" className="space-y-6 mt-6" data-testid="job-pl-report-content">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Stat label="Gross Profit" value={valueFormatter(jobPlReport?.total_profit || 0)} icon={TrendingUp} accent="emerald" note={`${jobPlReport?.total_margin_pct ?? 0}% margin`} />
            <Stat label="Jobs" value={jobPlReport?.jobs?.length || 0} icon={Briefcase} accent="blue" />
          </div>
          <Panel>
              ...
              <div className="overflow-x-auto -mx-6 px-6">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-slate-100">
                    {["Job ID", "Customer", "Orders", "Revenue", "COGS", "Profit", "Margin"].map((h) => <th key={h} className="text-left text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400 pb-3 pr-4 last:pr-0">{h}</th>)}
                  </tr></thead>
                  <tbody className="divide-y divide-slate-50">
                    {jobPlReport.jobs.map((job, i) => {
                      const isHigh = job.margin_pct >= 40; const isLow = job.margin_pct < 30;
                      return (
                        <tr key={i} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="py-3"><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold tabular-nums ${isHigh ? "bg-emerald-50 text-emerald-700" : isLow ? "bg-orange-50 text-orange-700" : "bg-blue-50 text-blue-700"}`}>{job.margin_pct}%</span></td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            ) : <div className="py-12 text-center"><Briefcase className="w-10 h-10 mx-auto text-slate-200 mb-3" /><p className="text-sm text-slate-400">No job data</p></div>}
          </Panel>
        </TabsContent>
      </Tabs>
    </div>
